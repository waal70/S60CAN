/* S60CAN (Open Source J2534 device) - Arduino firmware - version 0.4 alpha
**
** (c) 2015 by André de Waal
**
**
** Big portions Copyright (C) 2012 Olaf @ Hacking Volvo blog (hackingvolvo.blogspot.com)
    ...
    switch (*cmd_buf_pntr) {
        ...
        case SET_BTR:
           ...
           unsigned long bitrate = 16000000 / (2*(1+BRP)*(3+TSEG1+TSEG2));
            //char msg[32];
            //sprintf(msg,"bitrate command: %lu |", bitrate );
            //printf(msg);
        ...
        case OPEN_CAN_CHAN:
          ...
          //Andre delete the following lines, as it returned error
          //if (is_in_normal_mode())
          //  return ERROR;
        ...
        case LISTEN_ONLY:
            // return error if controller is not initialized or already open
            //Andre if not in config mode, put it in config mode...
            //if (get_operation_mode() != MODE_CONFIG)
            //  return ERROR;

            if (switch_mode(MODE_CONFIG))
              if (switch_mode(MODE_LISTENONLY))
                return CR;
              else
                return ERROR;
